...
import { useIntegrations, useIntegrationLogs, IntegrationWithProvider } from "@/hooks/useIntegrations";
...
export default function IntegracaoDetalhe() {
  ...
  const { integrations, loading, toggleIntegration, runIntegration } = useIntegrations(tenantId);
  ...
  if (loading || logsLoading) return <LoadingSkeleton variant="portal" />;
  ...
  const statusInfo = STATUS_BADGE[integration.last_status ?? ""] ?? { label: "N/A", variant: "outline" as const };
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <Button
                size="sm"
                onClick={() =>
                  runIntegration(integration.tenant_id, provider?.slug ?? integration.provider)
                }
